...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"tmep\":%.1f,",Data_init.temperatuer); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"mq135\":%d,",Data_init.mq135); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fire\":%d,",(u8)relay3in); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fan\":%d,",(u8)relay2in); 
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"people\":%d",device_state_init.poeple_state); 
	strcat(buf, text);	
...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"tmep_v\":\"%d\",",threshold_value_init.temp_value); 
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"mq135_v\":\"%d\",",threshold_value_init.mq135_value); 
	strcat(buf, text);
